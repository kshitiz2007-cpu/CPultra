...
export default function QuizPlayerPage() {
  ...
  useEffect(() => {
    async function initQuiz() {
      ...
      setTimeLeft(quizData.time_limit * 60); // Convert minutes to seconds[cite: 1]
      setLoading(false);
    }
  ...
  const handleSubmit = async () => {
    ...
    // Calculate score based on original logic[cite: 1]
    ...
    // Direct to a generic result view or back to dashboard
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 md:p-6 pb-24 min-h-screen flex flex-col">
      ...
      {hasHi && (
        <div className="flex items-center justify-between bg-orange-50/80 border border-orange-200 p-3 rounded-xl mb-4 backdrop-blur-sm animate-fade-in">
          ...
          <div className="flex gap-2">
            <button 
              onClick={() => setLang('en')}
              className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${lang === 'en' ? 'bg-white text-orange-600 shadow-sm' : 'text-orange-700/60 hover:bg-orange-100/50'}`}
            >
              EN
            </button>
            <button 
              onClick={() => setLang('hi')}
              className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${lang === 'hi' ? 'bg-white text-orange-600 shadow-sm' : 'text-orange-700/60 hover:bg-orange-100/50'}`}
            >
              हिंदी
            </button>
          </div>
      ...
      <div className="flex gap-4 animate-fade-in">
        {currentQ > 0 && (
          <button 
            ...
            className="flex-1 bg-white/60 hover:bg-white border-2 border-emerald-100 text-emerald-800 font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm"
          >
      ...
      <div className="mt-6 text-center">
        <button 
          onClick={() => {
            if (confirm('Abandon this quiz? Progress will be lost.')) router.push('/dashboard');
          }}
          className="text-sm font-semibold text-gray-400 hover:text-red-500 transition-colors uppercase tracking-wider"
        >
          Abandon Quiz
        </button>
      </div>
